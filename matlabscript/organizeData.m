function saveKinAggrogate(fileName)
    ...
    for k=1:length(cellType)
        ...
        if allNeurons || (~isempty(cellType{:,k}) && cellType{:,k} == "PC") 
            sumP = sumP + 1;
            binaryMatrix = [binaryMatrix, cellData(k).Bin1(:,2)];
    ...
    % save file 
    save("PC"+sumP+"trial_"+length(simStruct)+"_Cebra_scaled_bin1_complex_"+exportPrefex, "kinAggrogate");
    disp("PC"+sumP+"trial_"+length(simStruct)+"_Cebra_scaled_bin1_complex_"+exportPrefex);
end